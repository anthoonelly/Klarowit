...
Zasilane przez Anthropic Claude. Edytorska, refined-minimalist estetyka.
...
- `@anthropic-ai/sdk` — generacja przez tool_use (gwarantowany strukturalny JSON)
...
# Otwórz .env.local i wklej swój klucz z https://console.anthropic.com/
...
# Wklej klucz, wybierz "Production, Preview, Development", potwierdź.
...
3. **Environment Variables** — dodaj `ANTHROPIC_API_KEY`.
4. Deploy.
...
2. Upgrade do Pro.
3. Włącz Vercel Edge Runtime (nie wszystkie biblioteki obsługują).
...
```bash
ANTHROPIC_API_KEY=sk-ant-xxx           # wymagane
ANTHROPIC_MODEL=claude-sonnet-4-5      # opcjonalne, default
```

Dostępne modele:
- `claude-sonnet-4-5` — domyślny, najlepszy stosunek jakość/cena.
- `claude-opus-4-5` — najinteligentniejszy, wolniejszy, droższy.
- `claude-haiku-4-5` — najszybszy, tańszy, niższa jakość dokumentacji formalnej.
...
├── components/
│   ├── context-drawer.tsx         # drawer z instrukcjami i wzorcami
│   ├── doc-tabs.tsx               # tabsy dokumentów
│   ├── document-pane.tsx          # prawy pane router
│   ├── document-view.tsx          # render gotowego dokumentu
│   ├── empty-state.tsx
│   ├── generating-view.tsx        # animacja stages
│   ├── icons.tsx                  # SVG ikony
│   ├── regenerate-modal.tsx       # modal regeneracji
│   ├── topbar.tsx
│   ├── transcript-pane.tsx        # lewy pane: upload, textarea, doc-select
│   └── tweaks-panel.tsx           # popover wyglądu
├── hooks/
│   ├── use-generation.ts          # state machine generacji + SSE consumer
│   ├── use-local-storage.ts
│   └── use-tweaks.ts
├── lib/
│   ├── anthropic.ts               # SDK client + prompt builder + tool_use
...
### Strukturalny output przez tool_use

Zamiast prosić model o JSON w odpowiedzi, używamy
`tool_choice: { type: 'tool', name: 'submit_document' }`. To wymusza
poprawny strukturalny output zgodny z `input_schema` — bez ryzyka
malformed JSON.
...
puste — wtedy `lib/anthropic.ts → buildSystemPrompt` użyje samego
`docType.guidance`.

## Limity i koszty

- Generacja jednego dokumentu: ~3000–8000 tokenów input + ~2000–4000 output.
- Sonnet 4.5: ~$0.01–0.05 za pełen pakiet 7 dokumentów.
...
- Transkrypcje, instrukcje i wzorce zapisywane są **tylko lokalnie**
  ...
  zewnętrzne API poza Anthropic w momencie generacji.

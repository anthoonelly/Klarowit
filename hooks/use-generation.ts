...
interface UseGenerationOptions {
  ...
  onProjectId?: (projectId: string, projectName: string) => void;
}
  ...
  const handleEvent = useCallback(
    (event: SSEEvent, regenDocId?: DocTypeId) => {
      switch (event.type) {
        ...
        case 'project':
          optsRef.current.onProjectId?.(event.projectId, event.projectName);
          break;

        case 'balance':
          optsRef.current.onBalanceChange?.(event.balance);
          break;
        ...
        case 'doc_complete':
          ...
          // Auto-switch active to the freshly completed doc if user hasn't picked
          setActiveId((curr) => curr ?? event.docId);
          break;
    ...
    async (body: object, regenDocId?: DocTypeId) => {
      ...
      while (true) {
        ...
        try {
          ...
          if (ac.signal.aborted) return;
          const msg = err instanceof Error ? err.message : 'Błąd strumienia.';
          setStatus('error');
          setErrorMessage(msg);
          return;
        }
